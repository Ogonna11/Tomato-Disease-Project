...
1.) Log transform is applied to ratio scaled continuous predictors. We need to identify such variables in our data.

2.) The terms have to be added manually into the formula; there is no automatic way (such as "~ .") of adding all the variables. Here we need to add all 85 predictors manually.
...
barplot(coefs, las=2, xlim=c(0, 40), col=colors)
```
